...
struct ThreeByThreeChannelDims{
	std::size_t hhhDim;
	std::size_t pppDim;
};
...
class SPBasis{
	public:
		...
		std::size_t basisIndicator;
		std::size_t nSpstates;
		std::size_t nParticles;
		std::size_t nChannels;
		std::size_t n3BodyChannels;
		int ** indexMap;
		double * spEnergy;
		...
		ThreeByThreeChannelDims * threeByThreeChanDims;
		...
		/**
		 * This function checks if the symmetry exists
		 */
		virtual int checkSympqrs(std::size_t p, std::size_t q, std::size_t r, std::size_t s) = 0;
    ...
    // Figuring out what isn't getting deallocated
		void deallocate_sp_basis();
};
