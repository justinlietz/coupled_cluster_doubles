...
ElectronGasSPBasis::ElectronGasSPBasis(std::size_t basisIndicatorIn, double r_sIn, std::size_t tzMaxIn,
		std::size_t nShellsIn, std::size_t nParticleShellsIn){

	this->r_s = r_sIn;
	this->basisIndicator = basisIndicatorIn;
	this->tzMax = 1; // 1 for one species
	this->nShells = nShellsIn;
	this->nParticleShells = nParticleShellsIn;

//	printf("npShells: %zu\n", this->nParticleShells);
	if(this->tzMax == 1){
		this->chanTzMax = -2;
		this->modChanTzMax = 0;
	...
	for( int nx = -this->nParticleMax; nx <= this->nParticleMax; nx++){
		for( int ny = -this->nParticleMax; ny <= this->nParticleMax; ny++){
			for( int nz = -this->nParticleMax; nz <= this->nParticleMax; nz++){
				...
				if(nx*nx + ny*ny + nz*nz <= this->fermiEnergy){
					for( int sz = -1; sz <= 1; sz = sz+2){
						for( int tz = -1; tz <= this->tzMax - 1; tz = tz+2){
							this->nParticles++;
						} // end tz loop
...
// Using as a testbed for 3Body Stuff atm. Here and Dims.
void ElectronGasSPBasis::setUpChannelValues(){
	int channelNmax = 2*this->nMax;
	int channel3BNmax = 3*this->nMax;
	int channelTzMax = this->chanTzMax;
	// Currently BROKEN for symmetric matter
	int channel3bTzMax = this->chanTzMax-1;
	...
	this->n3BodyChannels = ((std::size_t)(2*channel3BNmax+1))
		* ((std::size_t)(2*channel3BNmax+1))
		* ((std::size_t)(2*channel3BNmax+1))
		* (4); //4 spin states, 1 tz state
	this->chanValue = new GasBundle[this->nChannels];
	this->chanModValue = new GasBundle[this->nChannels];
	this->chan3BValue = new GasBundle[this->n3BodyChannels];
	this->threeByThreeChanDims = new ThreeByThreeChannelDims[this->n3BodyChannels];
	//printf("nSpstates: %zu\n",this->nSpstates);
	//printf("n2Bchans: %zd\n",this->nChannels);
	//printf("n3Bchans: %zd\n",this->n3BodyChannels);
	...
	/*
	for(int ichanNx = -channel3BNmax; ichanNx <= channel3BNmax; ichanNx++){
		for(int ichanNy = -channel3BNmax; ichanNy <= channel3BNmax; ichanNy++){
			for(int ichanNz = -channel3BNmax; ichanNz <= channel3BNmax; ichanNz++){
				for(int ichanSz = -3; ichanSz <= 3; ichanSz = ichanSz +2){
					for(int ichanTz = -3; ichanTz <= -3; ichanTz = ichanTz +2){
						this->chan3BValue[channelCount].chanNx = ichanNx;
						this->chan3BValue[channelCount].chanNy = ichanNy;
						this->chan3BValue[channelCount].chanNz = ichanNz;
						this->chan3BValue[channelCount].chanSz = ichanSz;
						this->chan3BValue[channelCount].chanTz = ichanTz;
//						printf("chan: %zu, nx: %d, ny: %d, nz: %d, sz: %d, tz: %d\n",channelCount,ichanNx,ichanNy,ichanNz,ichanSz,ichanTz);
						channelCount++;
					}
				}
			}
		...
		// NEW SHIT. DELETE LATER
		for( std::size_t ichan = 0; ichan < this->n3BodyChannels; ichan++){
			this->threeByThreeChanDims[ichan].pppDim = 0;
			this->threeByThreeChanDims[ichan].hhhDim = 0;
		}

		for( int ichan = 0; ichan < this->n3BodyChannels; ichan++){
			for( int p = 0; p < this->nParticles; p++) {
				for( int q = 0; q < this->nParticles; q++) {
					for( int r = 0; r < this->nParticles; r++) {
						if( check3BChanSym(p, q, r, ichan) == 1 ){
							threeByThreeChanDims[ichan].hhhDim++;
						}
					}
				}
			}
		...
		for( int ichan = 0; ichan < this->n3BodyChannels; ichan++){
			for( int p = this->nParticles; p < this->nSpstates; p++) {
				for( int q = this->nParticles; q < this->nSpstates; q++) {
					for( int r = this->nParticles; r < this->nSpstates; r++) {
						if( check3BChanSym(p, q, r, ichan) == 1 ){
							threeByThreeChanDims[ichan].pppDim++;
						}
					}
				}
			}
	...
	int pppSum = 0;
		for( int ichan = 0; ichan < this->n3BodyChannels; ichan++){
			// printf("chan: %d, hhhDim: %zu, pppDim: %zu\n",ichan, threeByThreeChanDims[ichan].hhhDim, threeByThreeChanDims[ichan].pppDim);
			pppSum += threeByThreeChanDims[ichan].pppDim * threeByThreeChanDims[ichan].pppDim;
			hhhSum += threeByThreeChanDims[ichan].hhhDim * threeByThreeChanDims[ichan].hhhDim;
		}
	printf("pppSum: %d, hhhSum: %d\n", pppSum, hhhSum);
*/
	channelCount = 0;
...
double ElectronGasSPBasis::calcTBME(std::size_t p, std::size_t q, std::size_t r, std::size_t s){
	return this->calc_TBME_not_antisym(p,q,r,s) -
		this->calc_TBME_not_antisym(p,q,s,r);
}

double ElectronGasSPBasis::calc_TBME_not_antisym(std::size_t p, std::size_t q, std::size_t r, std::size_t s){
...
int ElectronGasSPBasis::spinExchangeMtxEle(int i, int j, int k, int l){
	if( i == l && j == k ){
		return 1;
	} else {
		return 0;
...
int ElectronGasSPBasis::kron_del(int i, int j){
	if(i != j){
		return 0;
	}
	return 1;
...
std::size_t ElectronGasSPBasis::TBchanIndexFunction(std::size_t p, std::size_t q){
...
std::size_t ElectronGasSPBasis::TBmodChanIndexFunction(std::size_t p, std::size_t q){
...
std::size_t ElectronGasSPBasis::keyGen_spHash(int nx, int ny, int nz, int sz, int tz){
...
std::size_t ElectronGasSPBasis::spIndex_from3Body(std::size_t q_inv, std::size_t r, std::size_t s){
	int nx, ny, nz, sz, tz;
...
int ElectronGasSPBasis::spIndexExists_from3Body(std::size_t q_inv, std::size_t r, std::size_t s){
	int nx, ny, nz, sz, tz;
	nx = this->indexMap[r][0] + this->indexMap[s][0] - this->indexMap[q_inv][0];
	ny = this->indexMap[r][1] + this->indexMap[s][1] - this->indexMap[q_inv][1];
	nz = this->indexMap[r][2] + this->indexMap[s][2] - this->indexMap[q_inv][2];
...
int ElectronGasSPBasis::checkSympqrs(std::size_t p, std::size_t q, std::size_t r, std::size_t s){
	printf("ElectronGasSPBasis::checkSympqrs should not be called\n");
	return 0;
}

int ElectronGasSPBasis::check3BChanSym(int p, int q, int r, int chan3B){
	int result = 0;
	int nx = this->indexMap[p][0] + this->indexMap[q][0] + this->indexMap[r][0];
	int ny = this->indexMap[p][1] + this->indexMap[q][1] + this->indexMap[r][1];
	int nz = this->indexMap[p][2] + this->indexMap[q][2] + this->indexMap[r][2];
	int sz = this->indexMap[p][3] + this->indexMap[q][3] + this->indexMap[r][3];
	int tz = this->indexMap[p][4] + this->indexMap[q][4] + this->indexMap[r][4];
	if( this->chan3BValue[chan3B].chanNx == nx ){
	if( this->chan3BValue[chan3B].chanNy == ny ){
	if( this->chan3BValue[chan3B].chanNz == nz ){
	if( this->chan3BValue[chan3B].chanSz == sz ){
	if( this->chan3BValue[chan3B].chanTz == sz ){
		result = 1;
	}}}}}
