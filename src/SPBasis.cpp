...
void SPBasis::deallocate_sp_basis(){
  delete[] this->chanDims;
  delete[] this->chanModDims;
  delete[] this->threeBodyChanDims;
  delete[] this->channelIndices;
  delete[] this->inverseChannelIndices;
  delete[] this->modChannelIndices;
  delete[] this->inverseModChannelIndices;
  delete[] this->spChannelIndices;
  delete[] this->inverseSPChannelIndices;


  for(std::size_t ichan=0; ichan < this->nChannels; ichan++){
    delete[] this->chanMaps[ichan].ppMap;
    delete[] this->chanMaps[ichan].hhMap;
    delete[] this->chanMaps[ichan].hpMap;
    delete[] this->chanMaps[ichan].phMap;

    delete[] this->chanModMaps[ichan].ppMap;
    delete[] this->chanModMaps[ichan].hhMap;
    delete[] this->chanModMaps[ichan].hpMap;
    delete[] this->chanModMaps[ichan].phMap;
  }

  for(std::size_t i = 0; i < this->nParticles; i++){
    delete[] this->threeBodyChanMaps[i].hppMap;
  }

  for(std::size_t a = this->nParticles; a < this->nSpstates; a++){
    delete[] this->threeBodyChanMaps[a].phhMap;
  }

  delete[] this->chanModMaps;
  delete[] this->chanMaps;
  delete[] this->threeBodyChanMaps;
}
...
void SPBasis::setUpChannelDims(){
  ...
  #pragma omp parallel
  {
    ...
    //printf("basisIndicator: %zu\n", this->basisIndicator);
    if( this->basisIndicator == 1 || this->basisIndicator == 2 ){
      ...
      for(std::size_t i = 0; i < this->nParticles; i++){
        for(std::size_t j = 0; j < this->nParticles; j++){
          for(std::size_t a = this->nParticles; a < this->nSpstates; a++){
            for(std::size_t b = this->nParticles; b < this->nSpstates; b++){
              if( this->checkSympqrs(i,j,a,b) == 1 ){
                this->threeBodyChanDims[i].hppDim++;
              }
              //if(spIndexExists_from3Body(j,a,b) && (sp_i = spIndex_from3Body(j,a,b)) < this->nParticles ){
              //	this->threeBodyChanDims[sp_i].hppDim++;
              //}
        ...
        for(std::size_t b = this->nParticles; b < this->nSpstates; b++){
          for(std::size_t i = 0; i < this->nParticles; i++){
            for(std::size_t j = 0; j < this->nParticles; j++){
              ...
              //	this->threeBodyChanDims[sp_a].phhDim++;
              //}
              if( this-> checkSympqrs(a,b,i,j) == 1 ){
                this->threeBodyChanDims[a].phhDim++;
              }
            } // end j
          } // end i
        } // end b
      } // end a
    } // end basisIndeciator else if
...
void SPBasis::setUpChannelMaps(){
  ...
  #pragma omp parallel
  {
    ...
    //printf("Made it to modified chans\n");
    // allocate maps
    #pragma omp for schedule(guided)
    for(std::size_t ichan=0; ichan < this->nChannels; ichan++){
    ...
    //printf("maps basis ind section reached\n");
    if( this->basisIndicator == 1 || this->basisIndicator == 2 ){
      // set
        ...
        for(std::size_t j = 0; j < this->nParticles; j++){
          for(std::size_t a = this->nParticles; a < this->nSpstates; a++){
            for(std::size_t b = this->nParticles; b < this->nSpstates; b++){
              ...
              //}
              if( this->checkSympqrs(i,j,a,b) == 1 ){
                atomicIdx = this->threeBodyChanDims[i].hppDim++;
                this->threeBodyChanMaps[i].hppMap[atomicIdx].p = j;
                this->threeBodyChanMaps[i].hppMap[atomicIdx].q = a;
                this->threeBodyChanMaps[i].hppMap[atomicIdx].r = b;
                //printf("i: %zu, idx: %zu\n\n", i, atomicIdx);
              }
            } // end b loop
          } // end a loop
        ...
        for(std::size_t b = this->nParticles; b < this->nSpstates; b++){
          for(std::size_t i = 0; i < this->nParticles; i++){
            for(std::size_t j = 0; j < this->nParticles; j++){
              ...
              //}
              if( this->checkSympqrs(a,b,i,j) == 1 ){
                atomicIdx = this->threeBodyChanDims[a].phhDim++;
                this->threeBodyChanMaps[a].phhMap[atomicIdx].p = b;
                this->threeBodyChanMaps[a].phhMap[atomicIdx].q = i;
                this->threeBodyChanMaps[a].phhMap[atomicIdx].r = j;
              }
            }
          }
        }
      }
    } // end basisIndicator if
